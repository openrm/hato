...
const TRANSIENT = { durable: false, exclusive: true };
...
describe('api', () => {
    ...
    const cases = {
        ...
        'an anonymous queue using the default exchange': {
            ...
            assert: (ctx, ch) => {
                ...
                assert.deepStrictEqual(ch.assertQueue.args[0], ['', TRANSIENT]);
            }
        },
        'a named queue using the default topic exchange': {
            do: (client) => client.type('topic').queue('a.queue', TRANSIENT),
            assert: (ctx, ch) => {
                assert.ok(ctx.assert);
        ...
        'an already declared queue': {
            before: (ch) => ch.assertQueue('a.declared', TRANSIENT),
            do: (client) => client
                .assert(false)
                .type('topic')
                .queue('a.declared'),
    ...
    it('should be allowed to reuse a queue already declared', (done) => {
        ...
        client
            .queue('foo', TRANSIENT); // Assert the queue first.
