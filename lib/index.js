...
class ContextChannel {
    ...
    consume(binding, fn, options) {
        ...
        return this._assert((ch) => bind(ch)
            .then(({ ch, queue }) => {
                return this._consume(ch, queue, fn, options).then((res) => {
                    subscription = res;
                    return ch;
                });
            }))
        ...
        return this.connected()
            .then((ch) => {
                return new Promise((resolve, reject) => {
                    ...
                    const ok = ch.publish(exchange, routingKey, msg, options, cb);
                    if (!ok) reject(new Error('Message could not be sent due to flow control'));
                });
            });
    }
...
const Client = function(url, options = {}) {
    ...
    const _assert = (fn) => asserted = fn(asserted);
